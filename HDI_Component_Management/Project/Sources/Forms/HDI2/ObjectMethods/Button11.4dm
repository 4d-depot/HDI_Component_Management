//SHOW ON DISK(Folder(fk database folder).file("Project/Sources/dependencies.json").platformPath)

//OPEN URL(Folder(fk database folder).file("Project/Sources/dependencies.json").platformPath)
...
While (Not:C34($envFile.exists) && Not:C34($envFolder=Null:C1517))
	...
	If (Not:C34($envFolder=Null:C1517))
		$envFile:=$envFolder.file("environment4d.json")
		OPEN URL:C673($envFolder.file("environment4d.json").platformPath)
	End if 
End while 
